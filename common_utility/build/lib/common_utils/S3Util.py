...
class S3Configuration(object):
    ...
    def __init__(self):
        ...
        if 'S3' in config.keys():
            if 'S3_local_folder_path' in config['S3'].keys() and config['S3']['S3_local_folder_path'] is not None:
                self.s3_local_folder_path= config['S3']['S3_local_folder_path']
            if 'default_bucket_name' in config['S3'].keys() and config['S3']['default_bucket_name'] is not None:
                self.default_bucket_name = config['S3']['default_bucket_name']
