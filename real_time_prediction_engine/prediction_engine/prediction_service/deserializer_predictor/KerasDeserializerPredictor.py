...
class KerasDeserializerPredictor(PipelinePredictionContext):
    ...
    def performPredictions(self,dataFrame,pipelineFilename,requestArguments):
        ...
        try:
            ...
            ServiceHelperFactory.fetchFileviaLocalApiFactory(predContext.pipeLineFileName)
            ServiceHelperFactory.fetchFileviaLocalApiFactory(predContext.modelFileName)
            ...
            with Lock():
                ...
                with session.as_default():
                        with graph.as_default():
                            predContext.loadedPipeLine.named_steps[Constants.PIPELINE_NAMED_STEP_MODEL].model = utils.load_h5_file(predContext.modelFileName)
                ...
                keras.backend.clear_session()
                logger.debug("Caching pipeline: " + str(utils.load_pipeline_cache.cache_info()))
                return predContext
